...
export class MangaStreamParser {
  ...
  parseSearchResults($: CheerioSelector, source: any): MangaTile[] {
    ...
    for (const manga of $("div.bs", "div.listupd").toArray()) {
      ...
      const image =
        this.getImageSrc($("img", manga))?.split("?resize")[0] ?? "";
  ...
  parseUpdatedManga(
    ...
    for (const manga of $(
      ...
      const id = this.idCleaner($("a", manga).attr("href") ?? "");
      const mangaDate = convertDateAgo(
        $("li > span", $("div.luf, ul.chfiv", manga)).first().text().trim(),
        source
      );
  ...
  parseHomeSections(
    ...
    for (const section of sections) {
      ...
      if (section.id == "new_titles") {
        ...
        if (
          !$("li", $(source.homescreen_NewManga_selector)?.parent()?.next())
            .length
        ) {
